...
<body class="bg-[#F4EFD8] w-full min-h-screen pt-20">
        ...
        <section class="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-20">
            ...
            <div class="space-y-6">
                ...
                <div class="flex space-x-3" id="gramasi-options">
                    <button data-gram="75" data-price="30000"
                        class="gramasi-btn py-2 px-6 border-2 border-green-700 bg-green-700 text-white rounded-full font-semibold text-sm">75 gram</button>
                    <button data-gram="200" data-price="65000"
                        class="gramasi-btn py-2 px-6 border-2 border-gray-300 bg-white text-gray-700 rounded-full font-semibold text-sm hover:border-green-700">200 gram</button>
                    <button data-gram="500" data-price="150000"
                        class="gramasi-btn py-2 px-6 border-2 border-gray-300 bg-white text-gray-700 rounded-full font-semibold text-sm hover:border-green-700">500 gram</button>
                </div>
        ...
        <section class="mt-16 pt-10 border-t border-gray-300">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div class="flex flex-col items-center md:items-start space-y-2">
                    <p class="text-6xl font-extrabold text-green-700">5.0<span class="text-3xl text-gray-500">/5</span>
                    </p>
                    ...
                    <p class="text-gray-500 text-sm">Based on <span class="font-semibold">15</span> reviews</p>
                </div>

                <div class="space-y-2 col-span-1 md:col-span-2">
                    <div class="flex items-center space-x-2">
                        <span class="text-sm font-semibold">5 Star</span>
                        <div class="flex-1 bg-gray-200 rounded-full h-2.5">
                            <div class="bg-green-700 h-2.5 rounded-full" style="width: 95%"></div>
                        </div>
                        <span class="text-sm text-gray-600">95%</span>
                    </div>
                    <div class="flex items-center space-x-2">
                        <span class="text-sm font-semibold">4 Star</span>
                        <div class="flex-1 bg-gray-200 rounded-full h-2.5">
                            <div class="bg-green-700 h-2.5 rounded-full" style="width: 5%"></div>
                        </div>
                        <span class="text-sm text-gray-600">5%</span>
                    </div>
                    <div class="flex items-center space-x-2">
                        <span class="text-sm font-semibold">3 Star</span>
                        <div class="flex-1 bg-gray-200 rounded-full h-2.5">
                    ...
                    <div class="flex items-center space-x-2">
                        <span class="text-sm font-semibold">2 Star</span>
                        <div class="flex-1 bg-gray-200 rounded-full h-2.5">
                            <div class="bg-green-700 h-2.5 rounded-full" style="width: 0%"></div>
                        </div>
                        <span class="text-sm text-gray-600">0%</span>
                    ...
                    <div class="flex items-center space-x-2">
                        <span class="text-sm font-semibold">1 Star</span>
                        <div class="flex-1 bg-gray-200 rounded-full h-2.5">
                            <div class="bg-green-700 h-2.5 rounded-full" style="width: 0%"></div>
                        </div>
                        <span class="text-sm text-gray-600">0%</span>
    ...
    <script>
        // Data Produk Dummy (untuk simulasi loadProductDetails)
        const allProducts = {
            'cashew-roasted-75': {
                name: 'ORINUTS KACANG PANGGANG ROASTED CASHEW (75gr)',
                price: 30000,
                description: 'Kacang mede panggang kualitas premium.',
                // Tambahkan detail lain untuk demo:
                gramasiOptions: [{ gram: 75, price: 30000 }, { gram: 200, price: 65000 }, { gram: 500, price: 150000 }]
            },
            'energy-booster-mix': {
                name: 'ORINUTS ENERGY BOOSTER MIX',
                price: 120000,
                description: 'Campuran energi dari kacang-kacangan dan buah kering.',
                gramasiOptions: [{ gram: 200, price: 120000 }]
            },
            'almond-roasted': {
                name: 'ORINUTS ALMOND PANGGANG',
                price: 40000,
                description: 'Almond utuh dipanggang tanpa garam.',
                gramasiOptions: [{ gram: 100, price: 40000 }, { gram: 300, price: 90000 }]
            },
        };
        ...
        // ============== 2. LOGIC GRAMASI/HARGA START ==============

        const priceDisplay = document.getElementById('product-price');
        const gramasiOptionsContainer = document.getElementById('gramasi-options');
        const gramasiButtons = document.querySelectorAll('.gramasi-btn');

        gramasiOptionsContainer.addEventListener('click', function (e) {
            const button = e.target.closest('.gramasi-btn');
            if (!button) return;

            // 1. Reset tampilan semua tombol
            gramasiButtons.forEach(btn => {
                btn.classList.remove('bg-green-700', 'text-white', 'border-green-700');
                btn.classList.add('bg-white', 'text-gray-700', 'border-gray-300');
            });

            // 2. Terapkan tampilan aktif pada tombol yang diklik
            button.classList.add('bg-green-700', 'text-white', 'border-green-700');
            button.classList.remove('bg-white', 'text-gray-700', 'border-gray-300');

            // 3. Ambil data harga dan ubah tampilan harga
            const newPrice = parseInt(button.dataset.price);
            priceDisplay.textContent = formatRupiah(newPrice);
        });

        // ============== 3. LOGIC WISHLIST START ==============

        const wishlistButton = document.getElementById('wishlist-button');
        const wishlistIcon = document.getElementById('wishlist-icon');
        const PRODUCT_ID = 'cashew-roasted-75'; // ID produk yang sedang ditampilkan
        let isWishlisted = false;

        wishlistButton.addEventListener('click', async () => {
            isWishlisted = !isWishlisted; // Toggle status

            // 1. Ubah tampilan ikon secara instan (Front-end)
            if (isWishlisted) {
                // Jika di-love, buat ikon terisi
                wishlistIcon.setAttribute('fill', 'currentColor');
                wishlistIcon.classList.remove('text-gray-400', 'hover:text-red-500');
                wishlistIcon.classList.add('text-red-500');
                console.log(`Produk ${PRODUCT_ID} ditambahkan ke Wishlist (Simulasi)`);
            } else {
                // Jika di-unlike, buat ikon kosong
                wishlistIcon.setAttribute('fill', 'none');
                wishlistIcon.classList.remove('text-red-500');
                wishlistIcon.classList.add('text-gray-400', 'hover:text-red-500');
                console.log(`Produk ${PRODUCT_ID} dihapus dari Wishlist (Simulasi)`);
            }

            // 2. Kirim permintaan ke Back-end (Simulasi Fetch API)
            // (Dalam pengembangan nyata, Anda akan mengganti ini dengan fetch())
            // try {
            //     const response = await fetch('/api/user/wishlist', {
            //         method: 'POST',
            //         body: JSON.stringify({ product_id: PRODUCT_ID, action: isWishlisted ? 'add' : 'remove' })
            //     });
        ...
        // ============== 4. LOGIC PRODUK LAINNYA (SIMULASI DINAMIS) ==============

        // Fungsi untuk mendapatkan Product ID dari URL
        function getProductIdFromUrl() {
            const params = new URLSearchParams(window.location.search);
            // Default ke 'cashew-roasted-75' (ID produk yang pertama kali ditampilkan)
            return params.get('id') || 'cashew-roasted-75';
        }
        ...
        function loadProductDetails(productId) {
            ...
            // 1. Perbarui Informasi Utama
            document.getElementById('product-title').textContent = productData.name;
            document.getElementById('product-price').textContent = formatRupiah(productData.price);

            // 2. Perbarui Pilihan Gramasi
            gramasiOptionsContainer.innerHTML = ''; // Kosongkan container lama
            productData.gramasiOptions.forEach((option, index) => {
                const button = document.createElement('button');
                button.setAttribute('data-gram', option.gram);
                button.setAttribute('data-price', option.price);
                button.classList.add('gramasi-btn', 'py-2', 'px-6', 'border-2', 'rounded-full', 'font-semibold', 'text-sm', 'hover:border-green-700');

                if (index === 0) { // Pilih gramasi pertama sebagai default
                    button.classList.add('bg-green-700', 'text-white', 'border-green-700');
                    // Perbarui harga display awal sesuai default
                    document.getElementById('product-price').textContent = formatRupiah(option.price);
                } else {
                    button.classList.add('bg-white', 'text-gray-700', 'border-gray-300');
                }
                button.textContent = `${option.gram} gram`;
                gramasiOptionsContainer.appendChild(button);
            });

            // Catatan: Dalam implementasi nyata, Anda akan memperbarui gambar dan deskripsi di sini.
        }

        // Panggil fungsi pemuatan detail saat halaman dimuat
        const initialProductId = getProductIdFromUrl();
        // Hanya jalankan jika halaman ini bukan halaman utama atau jika ada ID spesifik
        if (window.location.pathname.includes('product-detail.html')) {
            loadProductDetails(initialProductId);
        }

        // Anda harus me-re-query gramasiButtons setelah DOM diubah oleh loadProductDetails
        // Atau biarkan event listener di container (seperti di poin 2) yang menangani klik.
        // Dalam kode ini, event listener di container sudah diterapkan, jadi aman.
    </script>
</body>
